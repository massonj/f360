...
def ProcessComponentOccurence(occurence):        
    for body in occurence.component.bRepBodies:
        ...
        dimensions = tuple(sorted([boundingBox.height, boundingBox.length, boundingBox.width]))
...
def run(context):
    ...
    try:
        ...
        with open(fname, 'w', newline='') as output:
            ...
            writer.writerow(('no', 'quantity', 'mat', 'app', 'd1', 'd2', 'd3', 'components'))
            for i, ((dimensions,material,appearance), part) in enumerate(sorted(parts.items())):
                dimensions = tuple(map(lambda d: product.unitsManager.formatInternalValue(d), dimensions))
                writer.writerow((i+1, part['quantity']) + (material, appearance) + dimensions + (';'.join(part['names']),))
